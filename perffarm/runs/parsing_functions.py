...
def add_postgres_settings(hash_value, settings):
    settings_set = PostgresSettingsSet.objects.filter(
        settings_sha256=hash_value).get()
...
def parse_pgbench_statement_latencies(statement_latencies, pgbench_result_id):
    ...
    for statement in statements:
        ...
        try:
            pgbench_statement = PgBenchStatement.objects.filter(
                statement=text).get()
...
def parse_pgbench_log_values(result, values):
    ...
    for line in lines:
        ...
        date = datetime.utcfromtimestamp(
            int(results[0])).strftime("%Y-%m-%dT%H:%M:%S%z")
            ...
            if cpu_average:
                for key, value in cpu_average.items():
                    collectd_cpu = CollectdCpu(
                        epoch=float(key),
                        percent_user=float(
                            get_collectd_item('percent-user', value)),
                        percent_system=float(
                            get_collectd_item('percent-system', value)),
                        percent_idle=float(
                            get_collectd_item('percent-idle', value)),
                        percent_wait=float(
                            get_collectd_item('percent-wait', value)),
                        percent_nice=float(
                            get_collectd_item('percent-nice', value)),
                        percent_interrupt=float(
                            get_collectd_item('percent-interrupt', value)),
                        percent_softirq=float(
                            get_collectd_item('percent-softirq', value)),
                        percent_steal=float(
                            get_collectd_item('percent-steal', value)),
                        pgbench_result_id=pgbench_result_id,
                    )
        ...
        try:
            for key, value in processes.items():
                collectd_processes = CollectdProcess(
                    epoch=float(key),
                    fork_rate=int(
                        float(get_collectd_item('fork_rate', value))),
                    ps_state_running=int(
                        float(get_collectd_item('ps_state-running', value))),
                    ps_state_stopped=int(
                        float(get_collectd_item('ps_state-stopped', value))),
                    ps_state_sleeping=int(
                        float(get_collectd_item('ps_state-sleeping', value))),
                    ps_state_paging=int(
                        float(get_collectd_item('ps_state-paging', value))),
                    ps_state_blocked=int(
                        float(get_collectd_item('ps_state-blocked', value))),
                    ps_state_zombies=int(
                        float(get_collectd_item('ps_state-zombies', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in contextswitch.items():
                collectd_contextswitch = CollectdContextswitch(
                    epoch=float(key),
                    contextswitch=int(
                        float(get_collectd_item('contextswitch', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in ipc_shm.items():
                collectd_ipc_shm = CollectdIpcShm(
                    ...
                    bytes_total=int(
                        float(get_collectd_item('bytes-total', value))),
                    bytes_rss=int(
                        float(get_collectd_item('bytes-rss', value))),
                    bytes_swapped=int(
                        float(get_collectd_item('bytes-swapped', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in ipc_msg.items():
                collectd_ipc_msg = CollectdIpcMsg(
                    epoch=float(key),
                    count_space=int(
                        float(get_collectd_item('count-space', value))),
                    count_queues=int(
                        float(get_collectd_item('count-queues', value))),
                    count_headers=int(
                        float(get_collectd_item('count-headers', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in ipc_sem.items():
                collectd_ipc_sem = CollectdIpcSem(
                    epoch=float(key),
                    count_total=int(
                        float(get_collectd_item('count-total', value))),
                    count_arrays=int(
                        float(get_collectd_item('count-arrays', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in memory.items():
                collectd_memory = CollectdMemory(
                    epoch=float(key),
                    memory_free=int(
                        float(get_collectd_item('memory-free', value))),
                    memory_used=int(
                        float(get_collectd_item('memory-used', value))),
                    memory_cached=int(
                        float(get_collectd_item('memory-cached', value))),
                    memory_buffered=int(
                        float(get_collectd_item('memory-buffered', value))),
                    memory_slab_recl=int(
                        float(get_collectd_item('memory-slab_recl', value))),
                    memory_slab_unrecl=int(
                        float(get_collectd_item('memory-slab_unrecl', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in swap.items():
                collectd_swap = CollectdSwap(
                    epoch=float(key),
                    swap_free=int(
                        float(get_collectd_item('swap-free', value))),
                    swap_used=int(
                        float(get_collectd_item('swap-used', value))),
                    swap_cached=int(
                        float(get_collectd_item('swap-cached', value))),
                    swap_io_in=int(
                        float(get_collectd_item('swap_io-in', value))),
                    swap_io_out=int(
                        float(get_collectd_item('swap_io-out', value))),
                    pgbench_result_id=pgbench_result_id,
                )
        ...
        try:
            for key, value in vmem.items():
                collectd_vmem = CollectdVmem(
                    epoch=float(key),
                    vmpage_number_active_file=int(
                        float(get_collectd_item('vmpage_number-active_file', value))),
                    vmpage_number_inactive_file=int(
                        float(get_collectd_item('vmpage_number-inactive_file', value))),
                    vmpage_number_isolated_file=int(
                        float(get_collectd_item('vmpage_number-isolated_file', value))),
                    vmpage_number_active_anon=int(
                        float(get_collectd_item('vmpage_number-active_anon', value))),
                    vmpage_number_inactive_anon=int(
                        float(get_collectd_item('vmpage_number-inactive_anon', value))),
                    vmpage_number_isolated_anon=int(
                        float(get_collectd_item('vmpage_number-isolated_anon', value))),
                    vmpage_number_file_pages=int(
                        float(get_collectd_item('vmpage_number-file_pages', value))),
                    vmpage_number_file_hugepages=int(
                        float(get_collectd_item('vmpage_number-file_hugepages', value))),
                    vmpage_number_file_pmdmapped=int(
                        float(get_collectd_item('vmpage_number-file_pmdmapped', value))),
                    vmpage_number_kernel_stack=int(
                        float(get_collectd_item('vmpage_number-kernel_stack', value))),
                    ...
                    vmpage_number_dirty=int(
                        float(get_collectd_item('vmpage_number-dirty', value))),
                    vmpage_number_dirty_threshold=int(
                        float(get_collectd_item('vmpage_number-dirty_threshold', value))),
                    vmpage_number_dirty_background_threshold=int(
                        float(get_collectd_item('vmpage_number-dirty_background_threshold', value))),
                    vmpage_number_vmscan_write=int(
                        float(get_collectd_item('vmpage_number-vmscan_write', value))),
                    vmpage_number_vmscan_immediate_reclaim=int(
                        float(get_collectd_item('vmpage_number-vmscan_immediate_reclaim', value))),
                    vmpage_number_anon_pages=int(
                        float(get_collectd_item('vmpage_number-anon_pages', value))),
                    vmpage_number_anon_transparent_hugepages=int(
                        float(get_collectd_item('vmpage_number-anon_transparent_hugepages', value))),
                    vmpage_number_shmem=int(
                        float(get_collectd_item('vmpage_number-shmem', value))),
                    vmpage_number_shmem_hugepages=int(
                        float(get_collectd_item('vmpage_number-shmem_hugepages', value))),
                    vmpage_number_shmem_pmdmapped=int(
                        float(get_collectd_item('vmpage_number-shmem_pmdmapped', value))),
                    vmpage_number_writeback=int(
                        float(get_collectd_item('vmpage_number-writeback', value))),
                    vmpage_number_writeback_temp=int(
                        float(get_collectd_item('vmpage_number-writeback_temp', value))),
                    vmpage_number_free_pages=int(
                        float(get_collectd_item('vmpage_number-free_pages', value))),
                    vmpage_number_free_cma=int(
                        float(get_collectd_item('vmpage_number-free_cma', value))),
                    vmpage_number_bounce=int(
                        float(get_collectd_item('vmpage_number-bounce', value))),
                    vmpage_number_unevictable=int(
                        float(get_collectd_item('vmpage_number-unevictable', value))),
                    vmpage_number_page_table_pages=int(
                        float(get_collectd_item('vmpage_number-page_table_pages', value))),
                    vmpage_number_mapped=int(
                        float(get_collectd_item('vmpage_number-mapped', value))),
                    vmpage_number_zspages=int(
                        float(get_collectd_item('vmpage_number-zspages', value))),
                    vmpage_number_mlock=int(
                        float(get_collectd_item('vmpage_number-mlock', value))),
                    vmpage_number_unstable=int(
                        float(get_collectd_item('vmpage_number-unstable', value))),
                    vmpage_action_written=int(
                        float(get_collectd_item('vmpage_action-written', value))),
                    vmpage_action_dirtied=int(
                        float(get_collectd_item('vmpage_action-dirtied', value))),
                    pgbench_result_id=pgbench_result_id,
                )
...
def parse_pgbench_custom_results(initsql_custom, custom_query1, pgbbench_result_id):
    ...
    try:
        if (InitSql.objects.filter(InitSql_sha256=hash_initsql).exists() == False):
            InitSql.objects.create(
                InitSql_sha256=hash_initsql, data=initsql_custom)

        InitSql_obj = InitSql.objects.filter(InitSql_sha256=hash_initsql).get()
        PgbenchCustomDetails.objects.create(
            init_sql=InitSql_obj, pgbench_result_id=pgbbench_result_id)
    except Exception as e:
        raise RuntimeError(e)
    ...
    try:
        for query in custom_query1:
            ...
            if (custom_query.objects.filter(custom_querys_sha256=hash_custom_querys).exists() == False):
                custom_query.objects.create(
                    custom_querys_sha256=hash_custom_querys, data=query['query'], weight=query['weight'])
            custom_querys_obj = custom_query.objects.filter(
                custom_querys_sha256=hash_custom_querys).get()
            custom_queries.objects.create(
                custom_query=custom_querys_obj, pgbench_result_id=pgbbench_result_id)
    except Exception as e:
        raise RuntimeError(e)
    ...
    for client in item['clients']:
        ...
        for result in json:
            ...
            if int(result['clients']) == client:
                ...
                pgbench_result_last = PgBenchResult.objects.order_by(
                    '-pgbench_result_id').first()
                ...
                try:
                    ...
                    if (item.get('custom_queries') != None):
                        parse_pgbench_custom_results(
                            item['init_sql'], item['custom_queries'], result_object)
                    ...
                    for row in avg_same_config_result_raw:
                        for column in avg_same_config_result_raw.columns:
                            avg_same_config_result[column] = getattr(
                                row, column)
                    ...
                    parse_pgbench_statement_latencies(
                        statement_latencies, result_object)
...
def parse_tpch_results(new_run, tpch_config, qphh_score, power_score, throughput_score, power_results, throughput_results, user):
    ...
    tpch_config = TpchConfig.objects.filter(
        scale_factor=tpch_config.scale_factor).get()
    ...
    try:
        ...
        try:
            ...
            save_tpch_query_result(
                throughput_results, 'throughput', result_object)
        except Exception as e:
        ...
        if avg_same_config_result and avg_same_config_result[
            ...
            send_the_email(recipents=[
                           user.email], subject="Performance Drop Alert —— Pgperffarm ", message=message)
...
def parse_os_kernel(json_data):
    try:
        os_distributor = OsDistributor.objects.filter(
            dist_name=json_data['os_information']['distributor']).get()

    except OsDistributor.DoesNotExist:

        try:

            os_distributor = OsDistributor(
                dist_name=json_data['os_information']['distributor'])
            os_distributor.save()

        except Exception as e:
            raise RuntimeError(e)
    try:
        os_kernel = Kernel.objects.filter(
            kernel_name=json_data['kernel']['uname_s']).get()
...
def parse_git(json_data):
    ...
    try:
        branch = Branch.objects.filter(
            name=json_data['git']['branch'], git_repo=repo.git_repo_id).get()
...
def parse_postgres(json_data):
    postgres_hash, postgres_hash_object = get_hash(
        json_data['postgres_settings'])

    try:
        postgres_info = PostgresSettingsSet.objects.filter(
            settings_sha256=postgres_hash).get()
...
def parse_explain_results(data, new_run, tpch_config, qphh_score, power_score, throughput_score):
    ...
    for key in data:
        ...
        explain_result = ExplainQueryCostOnResult(tpch_query=tpch_query, tpch_result=result_object,
                                                  planning_time=new_data["Planning Time"], execution_time=new_data["Execution Time"])
        try:
            explain_result.save()
        except Exception as e:
            raise RuntimeError(e)
        for x in list11:
            result_object = ExplainQueryCostOnResultDetails(
                explain_query_cost_on_result=explain_result, result=x)
            try:
                result_object.save()
            except Exception as e:
                raise RuntimeError(e)
...
def parse_explain_results_costOff(data, new_run, tpch_config, qphh_score, power_score, throughput_score):
    ...
    for key in data:
        ...
        result_object = TpchResult.objects.filter(run_id=new_run, benchmark_config=tpch_config, power_score=power_score,
                                                  throughput_score=throughput_score, composite_score=qphh_score).first()
        query_result_object = TpchQueryResult.objects.filter(
            query_idx=int(key), tpch_result=result_object).first()

        for x in list11:
            hash = hashlib.sha256(str(x).encode('utf-8')).hexdigest()
            query_plan_result = ExplainQueryCostOffPlan.objects.filter(
                hash=hash)
            if (query_plan_result == None or query_plan_result.count() == 0):
                query_plan_result = ExplainQueryCostOffPlan(
                    hash=hash, result=x)
                try:
                    query_plan_result.save()
                except Exception as e:
                    raise RuntimeError(e)
            plan_result = ExplainQueryCostOffPlan.objects.filter(
                hash=hash).first()
            result_object = ExplainQueryCostOffResult(
                tpch_query=query_result_object, plan_hash=plan_result)
            try:
                result_object.save()
            except Exception as e:
                raise RuntimeError(e)
